...
class MyCode implements IParser
{
    ...
    public function parsePlain($message, $options = array())
    {
        ...
        $this->list_count = 0;
        // Find all lists
        $message = preg_replace_callback("#(\[list(=(a|A|i|I|1))?\]|\[/list\])#si", array($this, 'prepareList'),
                                         $message);
        // Replace all lists
    ...
    private function parseMyCode($message)
    {
        ...
        if ($this->mycode_cache['standard_count'] > 0) {
            $message = preg_replace($this->mycode_cache['standard']['find'],
                                    $this->mycode_cache['standard']['replacement'], $message);
        }
        ...
        if ($this->allowvideocode) {
            $message = preg_replace_callback("#\[video=(.*?)\](.*?)\[/video\]#i", array($this, 'parseVideoCallback'),
                                             $message);
        } else {
    ...
    private function autoUrl($message)
    {
        ...
        $message = preg_replace_callback("#([\>\s\(\)])(http|https|ftp|news|irc|ircs|irc6){1}://([^\/\"\s\<\[\.]+\.([^\/\"\s\<\[\.]+\.)*[\w]+(:[0-9]+)?(/([^\"\s<\[]|\[\])*)?([\w\/\)]))#iu",
                                         array(
                                             $this,
                                             'autoUrlCallback'
                                         ), $message);
        $message = preg_replace_callback("#([\>\s\(\)])(www|ftp)\.(([^\/\"\s\<\[\.]+\.)*[\w]+(:[0-9]+)?(/([^\"\s<\[]|\[\])*)?([\w\/\)]))#iu",
                                         array(
                                             $this,
                                             'autoUrlCallback'
                                         ), $message);
    ...
    private function ParsePostQuotes($message, $username, $text_only = false)
    {
        ...
        if (!empty($this->postURL)) {
            ...
            if (isset($match[1]) && (int) $match[1]) {
                ...
                $username = preg_replace("#(?:&quot;|\"|')? pid=(?:&quot;|\"|')?[0-9]+[\"']?(?:&quot;|\"|')?#i", '',
                                         $username);
                $delete_quote = false;
            }
            unset($match);
        ...
        if (isset($match[1]) && (int) $match[1]) {
            ...
            $username = preg_replace("#(?:&quot;|\"|')? dateline=(?:&quot;|\"|')?[0-9]+(?:&quot;|\"|')?#i", '',
                                     $username);
            $delete_quote = false;
        }
        if ($delete_quote) {
